...
def markdownify_pandoc(html):
    p = subprocess.Popen(['pandoc', '--normalize',
        '-f', 'html', '-t', 'markdown_github', '-'],
        stdin=subprocess.PIPE,
        stdout=subprocess.PIPE)
    stdout, stderr = p.communicate(input=html.encode('utf-8'))
...
def update_content(entry):
    ...
    content = CSSSelector('section.posttext')(doc)[0]
...
def write_entry(entry, data, opts):
    ...
    with open(os.path.join(opts.output_dir, '%s-%s.md' % (data["date"], data['slug'])), 'w') as fd:
